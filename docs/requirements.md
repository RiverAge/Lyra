...
```
<音乐库根>/.lyrics/apple/Artist/Album/01 Song.ttml        ← Apple 原始 TTML（默认主歌词）
<音乐库根>/.lyrics/netease/Artist/Album/01 Song.json      ← 网易原始歌词 JSON
<音乐库根>/.lyrics/qq/Artist/Album/01 Song-qq.ttml         ← QQ QRC 逐字
<音乐库根>/.lyrics/netease/Artist/Album/01 Song-netease.ttml ← 网易逐字增强
```

sidecar 与音频同构镜像 `<音乐库根>/apple` 下相对路径。设计哲学见 `AppleMusicDecrypt-Windows/docs/lyrics-workflow.md`。

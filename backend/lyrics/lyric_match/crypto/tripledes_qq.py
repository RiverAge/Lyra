# ruff: noqa: UP009, E501 - 逐字从 LDDC/GPL-3.0 移植（§3.9 非标准 3DES，
# 不得换库）；S-box/置换表排列保留原码，行长不重排
# -*- coding: utf-8 -*-
"""QQ QRC 3DES decryption.

Lifted verbatim from LDDC (https://github.com/chenmozhijin/LDDC,
`LDDC/core/decryptor/tripledes.py`, GPL-3.0-only), which itself ports the C#
QQMusicDecoder (https://github.com/WXRIW/QQMusicDecoder). Two changes only:

  1. Dropped `from LDDC.common.data.cache import cache` and the
     `@cache.memoize()` decorator on `tripledes_key_setup` — this module has
     no cache dependency so it runs standalone.
  2. Added `decrypt_qrc(hex_str)` — the high-level wrapper that turns the
     hex ciphertext from `lyric_download.fcg`'s `<content>` CDATA into the
     decrypted QRC XML string.

Why not PyCryptodome? QQ's "3DES" is a hand-written non-standard bit
permutation (the S-box layout / key schedule differ from standard DES);
`Crypto.Cipher.DES3` MODE_ECB produces garbage (zlib header check fails).
Only this port is verified to decrypt real QRC (周杰伦《晴天》, 2026-07-09).

License note: LDDC is GPL-3.0-only (infectious). This copy is used in a
private, non-distributed deployment — no distribution occurs, so the
infectiousness has no effect. If this code is ever redistributed, the whole
work must ship under GPL-3.0; re-evaluate then (an Apache-2.0 re-implementation
from WXRIW/QQMusicDecoder is the escape hatch).

Original headers preserved below:
    # 参考原C#代码:
    # https://github.com/WXRIW/QQMusicDecoder/blob/.../QQMusicDecoder/DESHelper.cs
    # https://github.com/WXRIW/QQMusicDecoder/blob/.../QQMusicDecoder/Decrypter.cs
...
def f(state: int, key: list[int]) -> int:
    ...
    # 与密钥进行异或运算
    lrgstate = [lrgstate[i] ^ key[i] for i in range(6)]  # type: ignore[assignment]
...
# ---- high-level wrapper (added for lyric_match; not part of LDDC) ----

# QQ's fixed 3DES key (24 bytes). Same value used across all QRC payloads.
_QQ_3DES_KEY = b"!@#)(*$%123ZXC!@!@#)(NHL"

# Decrypt key schedule is constant — compute once at module load.
_DECRYPT_SCHEDULE = tripledes_key_setup(_QQ_3DES_KEY, DECRYPT)


def decrypt_qrc(hex_str: str) -> str:
    """Decrypt a QQ QRC ciphertext hex string into the QRC XML string.

    hex_str is the CDATA payload of `<content>` from `lyric_download.fcg`
    (uppercase hex). Pipeline: hex→bytes→8-byte block 3DES decrypt→zlib
    inflate→utf-8. Raises ValueError on malformed hex / bad zlib (usually a
    sign the ciphertext is not a QRC, e.g. an empty `contentts`).
    """
    data = bytes.fromhex(hex_str)
    schedule = _DECRYPT_SCHEDULE
    out = bytearray()
    for i in range(0, len(data) - len(data) % 8, 8):
        out.extend(tripledes_crypt(bytearray(data[i:i + 8]), schedule))
    decompressed = zlib.decompress(bytes(out))
